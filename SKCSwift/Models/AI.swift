...
- Always split clauses where one self-contained clause ends and a different one begins. 
- Sometimes clauses are multi-sentence and terminated with a period (though not always). It is your job to determine where the full clause terminates.
...
- An ingredient clause will almost never be the only clause.
- YOU MUST USE EXHAUST WORD OF THE PROVIDED TEXT — concatenating all clauses in order must reproduce the original text. This is a partition of the text, not a rewrite.

EXAMPLE 1: Text with ingredient list
...
EXAMPLE 2: Multiple clauses; the trailing limiter is its own element.
...
EXAMPLE 3: Bulleted clause stays as one element; trailing limiter separate.
